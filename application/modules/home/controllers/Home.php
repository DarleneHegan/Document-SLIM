    ...
    public function index() {
     ...
     public function detail($apps_id = 0, $service_id = 0) {
        ...
        if ($apps_id > 0) {
            ...
            $data['sup_doc_history'] = $m->get_sup_history($apps_id);
...
// =========================================================================
    ...
    public function upload_sup_doc($apps_id) {
        ...
        if (!$this->upload->do_upload('sup_doc_file')) {
            ...
            $insert_result = $this->db->insert('tbl_apps_audit_trail', [
                'apps_id'    => (int)$apps_id,
    ...
    public function download_sup_doc($file_name) {
        $file_name = $this->security->sanitize_filename($file_name);
        $this->load->helper('download');
        $path = './uploads/documents/' . $file_name;
