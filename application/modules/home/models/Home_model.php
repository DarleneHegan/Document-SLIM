...
class Home_model extends CI_Model {
    ...
    public function get_sup_history($apps_id) {
        $this->db->where('apps_id', $apps_id);
        $this->db->order_by('version', 'DESC'); 
        
        if ($this->db->table_exists('tbl_apps_sup_documents')) {
            return $this->db->get('tbl_apps_sup_documents')->result_array();
        }
        return [];
    }

    public function insert_sup_history($apps_id, $filename, $ext, $remarks = 'Uploaded Supporting Document') {
        if (!$this->db->table_exists('tbl_apps_sup_documents')) return false;

        // Cari versi terakhir
        $this->db->where('apps_id', $apps_id);
        $this->db->select_max('version');
        $row = $this->db->get('tbl_apps_sup_documents')->row();
        
        $new_version = ($row && $row->version) ? $row->version + 1 : 1;

        $this->db->insert('tbl_apps_sup_documents', [
            'apps_id'     => $apps_id,
            'version'     => $new_version,
            'file_name'   => $filename,
            'file_format' => $ext, 
